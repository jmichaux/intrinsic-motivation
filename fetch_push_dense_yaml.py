...
parser.add_argument('--experiment-name', nargs='+', type=str,
                    default=["RandomAgent"])
parser.add_argument('--env-name', nargs='+', type=str,
                    default=["FetchPushDense-v1"])
...
# command template
template = "python main.py --experiment-name {0} --env-name {1} --num-env-steps 100000 --num-processes 1 --num-steps 2048 --ppo-epoch 10 --num-mini-batch 32 --hidden-size {2} --pi_lr {3} --clip-param {4} --value-loss-coef {5} --entropy-coef {6} --grad-norm-max {7} --gamma {8} --gae-lambda {9} --use-gae --share-optim  use-clipped-value-loss --log-interval 1 --use-tensorboard"
#
config = {"session_name": "run-all", "windows": []}

panes_list = []
for element in itertools.product(*arg_list):
        panes_list.append(template.format(*element))
config["windows"].append({"window_name": "{}/{}".format(args.experiment_name, args.env_name), "panes": panes_list})
